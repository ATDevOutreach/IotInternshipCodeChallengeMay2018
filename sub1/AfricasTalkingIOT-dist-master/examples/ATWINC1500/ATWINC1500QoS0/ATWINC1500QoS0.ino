/*
 Name:		ATWINC1500QoS0.ino
 Created:	11/1/2017 3:52:10 PM
...
void loop() {
	...
	if (!client.loop()) {
		...
		if (client.connect("ATWIC1500QoS0", "<username>", "<password>")) {
	...
	if (now - lastMsg > 5000) {
		...
		Serial.println(msg);
		client.publish("<username>/ATWIC1500QoS0/readings", msg, false);
	}
}
...
void _keepAlive() {
	...
	while (!client.connected()) {
		...
		if (client.connect("ATWIC1500QoS0", "<username>", "<password>")) {
			...
			client.publish("<username>/ATWIC1500QoS0/birth", "ATWIC1500QoS2");
			client.subscribe("<username>/ATWIC1500QoS0/commands");
		}
		else {
