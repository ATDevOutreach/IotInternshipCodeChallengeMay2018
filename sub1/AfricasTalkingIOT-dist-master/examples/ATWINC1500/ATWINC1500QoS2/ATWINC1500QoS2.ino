/*
 Name:		ATWINC1500QoS2.ino
 Created:	11/1/2017 3:44:39 PM
...
void loop() {
	...
	if (!client.loop()) {
		...
		if (client.connect("ATWIC1500QoS2", "<username>", "<password>")) {
	...
	if (now - lastMsg > 5000) {
		...
		Serial.println(msg);
		client.publish("<username>/ATWIC1500QoS2/readings", msg, 2, false);
	}
}
...
void _keepAlive() {
	...
	while (!client.connected()) {
		...
		if (client.connect("ATWIC1500QoS2", "<username>", "<password>")) {
			...
			client.publish("<username>/ATWIC1500QoS2/birth", "ATWIC1500QoS2");
			client.subscribe("<username>/ATWIC1500QoS2/commands");
		}
		else {
